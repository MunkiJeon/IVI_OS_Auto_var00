...
class VehicleControlPage(BasePage):
    ...
    def reset_sidebar(self):
        try:
            print("Resetting sidebar scroll to top...")
            self.driver.find_element(AppiumBy.ANDROID_UIAUTOMATOR, 'new UiScrollable(new UiSelector().scrollable(true).instance(0)).scrollToBeginning(5)')
            time.sleep(1)
        except Exception as e:
            print(f"Sidebar reset failed: {e}")
