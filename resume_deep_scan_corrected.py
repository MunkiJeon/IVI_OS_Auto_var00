...
def finish_remaining_discovery_resumed():
    try:
        d = DriverFactory.get_driver()
        p = VehicleControlPage(d)
        
        # Remaining 4 menus (Using CORRECTED sidebar text)
        menus = ['편의 기능', '개인정보 보호', '보안', '차량 정보']
        
        for menu in menus:
            print(f">>> FINAL SCAN RESUMED: {menu} <<<")
            
            # Reset and scan sidebar (scrolling up to 10 times)
            found_menu = False
            for _ in range(5): d.swipe(800, 200, 800, 800, 400) # Reset to top
            ...
            # Save results
            with open(f"final_resumed_corrected_{menu}.txt", "w", encoding="utf-8") as f:
                for it in p_items:
                    f.write(f"Item: {it['text']} | Desc: {it['desc']} | Class: {it['class']} | Bounds: {it['bounds']}\n")
            
            print(f"  - Captured {len(p_items)} clickable items for {menu}.")
            d.save_screenshot(f"final_resumed_corrected_{menu}.png")
...
if __name__ == "__main__":
    finish_remaining_discovery_resumed()
