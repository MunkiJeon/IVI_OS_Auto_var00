...
def finish_discovery_v2():
    try:
        d = DriverFactory.get_driver()
        p = VehicleControlPage(d)
        
        # Remaining 4 menus (Using exact sidebar text)
        menus = ['편의', '개인정보 보호', '보안', '차량 정보']
        
        for menu in menus:
            print(f">>> FINAL SCAN V2: {menu} <<<")
            
            # Reset and scan sidebar (scrolling up to 10 times)
            found_menu = False
            for _ in range(5): d.swipe(800, 200, 800, 800, 400) # Full reset to top
            
            for s_idx in range(15):
                source = d.page_source
                if menu in source:
                    # Found it, but maybe it's partially off-screen. Center it.
                    p.click_sidebar_menu(menu)
                    found_menu = True
                    break
                # Scroll a bit more aggressively
                d.swipe(820, 800, 820, 300, 800)
                time.sleep(1)
